...
任一文件未抽出可核验的含税金额时，服务端会再按整页图片调用多模态模型核对；若初次 OCR 没有文本，会先重读图片取得可溯源文本。仍无法取得金额则任务进入 `failed`，`error` 说明原因，不会把缺失金额作为 0 返回。明确识别出的 0 元仍是有效金额。

含税合计优先取明确的“价税合计/含税金额”列；没有该列时，若“不含税金额”和“税额”两列都抽到数值，由代码相加。税额列存在但数值缺失时会继续核对，不把缺失税额当作 0。
