...
callback_secret: HMAC-SHA256 验签密钥
```
...
- `GET /api/v1/external/compare/{task_id}`：任务状态与完整外部结果。
- `GET /api/v1/external/compare/{task_id}/images/{page_number}`：指定页 PNG。

上述查询和图片端点同样要求 `X-API-Key`。回调带 `X-Signature` 和
`X-Event-Id`；签名是以调用方提交的 `callback_secret` 对原始 JSON body 做
HMAC-SHA256。回调地址按当前内网集成要求允许 HTTP/HTTPS 内网地址，因此
...
外部集成使用 `POST /api/v1/external/compare`，完成回调直接提供确定性结果文本、
