...
callback_secret: HMAC-SHA256 验签密钥(可选,留空则回调不带 X-Signature 签名)
```

接口返回 HTTP 202 与 `task_id`。任务完成后回调 `result.result_text`、识别与
定位状态、所有页面的高亮 PNG 下载链接；相同 `document_no` 允许多次提交。
外部系统也可通过以下端点查询：
...
上述查询和图片端点同样要求 `X-API-Key`。回调带 `X-Signature` 和
`X-Event-Id`；签名是以调用方提交的 `callback_secret` 对原始 JSON body 做
HMAC-SHA256。`callback_secret` 可选:留空(或空白)时回调仍按 `callback_url`
正常投递,但请求头中省略 `X-Signature`,由调用方自行决定是否校验完整性。
