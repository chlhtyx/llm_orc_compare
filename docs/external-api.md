...
2. 校验等待队列容量(见下)。
3. 创建任务(生成 `task_id`),保存上传文件或落定下载产物,持久化入队。
4. 根据 `sync` 走同步等待分支,或立即返回受理响应。

#### 并发限制

服务端最多同时执行 `DC_MAX_CONCURRENT_TASKS` 个任务；达到执行上限后的任务进入 PostgreSQL 持久化队列。等待队列达到 `DC_MAX_QUEUED_TASKS` 时才返回 **429**:`等待队列已满,请稍后重试`。建议外系统对 429 做退避重试。

---

## 三、同步调用模式 (`sync=true`)

> 适合单次即时调用。调用方优先等待至比对完成并在同一个 HTTP 响应里拿到完整结果；若任务持续处于等待队列并超过 `DC_SYNC_QUEUE_WAIT_SECONDS`，接口返回 `202 + task_id`，任务继续执行，可查询或等待回调。
...
同步请求在尚未获得执行槽位时最多等待 `DC_SYNC_QUEUE_WAIT_SECONDS`；超过该时间返回 `202 + task_id`，任务继续执行。已开始执行的同步请求仍等待 pipeline 完成；典型耗时从数十秒到数分钟不等(取决于 PDF 页数、是否走 OCR、是否启用 LLM 辅助)。
