...
- 外部比对 HTML 报告产物:外部合同比对任务完成时,除高亮 PNG 外,额外生成自包含 HTML 报告(`external_api.write_external_html_report` → `reports_dir/{task_id}_external_report.html`,渲染在 `report/html_report.py`),作为 `result_url` 的下载目标。报告含结构化差异表格 + 已烧录差异标注的回收件每页 PNG(base64 data URI 内嵌,单文件离线可看,不依赖图片端点)。`result_url` 由原 JSON 查询端点**改为**指向 `GET /api/v1/external/contractCompare/{task_id}/report.html`(attachment 下载,文件名 `【单据号】对比YYYYMMDD-HHMM.html`,北京时间为完成时刻到分钟;on-disk 仍按 task_id 命名)。渲染失败只记日志不阻塞任务主流程。JSON 查询端点 `GET /contractCompare/{task_id}` 保留不变。
